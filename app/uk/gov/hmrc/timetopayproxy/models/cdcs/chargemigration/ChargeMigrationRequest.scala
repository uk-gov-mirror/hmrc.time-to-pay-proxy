...
case class ChargeMigrationRequest(
  planId: String,
  migratedAt: Instant,
  planCreationChannel: ChannelIdentifier,
  chargeMigrations: List[ChargeMigration]
)

object ChargeMigrationRequest {
  implicit val format: Format[ChargeMigrationRequest] =
    Json.format[ChargeMigrationRequest]
}
